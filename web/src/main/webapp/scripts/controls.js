...
Ajax.InPlaceEditor = Class
		.create({
			...
			postProcessEditField : function() {
				...
				if (fpc) {
					$(this._controls.editor)['focus' == fpc ? 'focus'
							: 'activate']();
				}
			},
			...
			registerListeners : function() {
				...
				$H(Ajax.InPlaceEditor.Listeners).each(
						function(pair) {
							...
							if (this.options.externalControl) {
								this.options.externalControl.observe(pair.key,
										listener);
							}
						}.bind(this));
			},
			...
			unregisterListeners : function() {
				$H(this._listeners).each(
						function(pair) {
							if (!this.options.externalControlOnly) {
								this.element
										.stopObserving(pair.key, pair.value);
							}
							if (this.options.externalControl) {
		...
		.create(
				Ajax.InPlaceEditor,
				{
					...
					showLoadingText : function(text) {
						...
						tempOption.update((text || '').stripScripts()
								.stripTags());
					},
					...
					buildOptionList : function() {
						...
						this._collection.each(function(entry, index) {
							...
							option.selected = textFound ? entry[0] == marker
									: 0 == index;
							option.appendChild(document
									.createTextNode(entry[1]));
...
Object.extend(Ajax.InPlaceEditor, {
	DefaultOptions : {
		...
		okText : 'ok',
		paramName : 'value',
		rows : 1, // If 1 and multi-line, uses
		// autoRows
