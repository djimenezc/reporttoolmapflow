package com.mapflow.webapp.spring;
...
/**
 * <p>
 * Adds commons validator configuration files to an existing Spring commons Validator Factory bean,
 * possibly defined within a seperate Spring configuration file in a seperate jar file. By using
 * this extension factory developers can add validation configuration for their own persistent
 * classes to an AppFuse application without modifying any of the existing AppFuse Spring
 * configuration or jar distribution files.
 * <p>
 * As an example consider the following Spring bean configuration:
 * 
 * <pre>
 * &lt;bean class=&quot;com.mapflow.spring.ValidatorExtensionPostProcessor&quot;&gt;
 *   &lt;property name=&quot;validationConfigLocations&quot;&gt;
 *     &lt;list&gt;
 *       &lt;value&gt;/WEB-INF/spring/foo-validation.xml&lt;/value&gt;
 *     &lt;/list&gt;
 *   &lt;/property&gt;
 * &lt;/bean&gt;
 * </pre>
 * <p>
 * The sample adds a single validation configuration file (foo-validation.xml) to an existing Spring
 * commons Validator Factory bean (a bean of class
 * org.springmodules.validation.commons.DefaultValidatorFactory). Assuming the validator extension
 * is included in a Spring configuration file called applicationContext-foo-validation.xml, and that
 * this configuration file is added directly below WEB-INF in the Foo web project, then the normal
 * war overlay process coupled with AppFuse's configuration file auto detection will ensure that the
 * validation extension is automatically included into the application without requiring any
 * modification of AppFuse's existing config files.
 * 
...
public class ValidatorExtensionPostProcessor implements BeanFactoryPostProcessor {
  ...
  /**
   * Adds the validation configuration files to the list already held in the validator factory bean
   * configuration.
   * 
