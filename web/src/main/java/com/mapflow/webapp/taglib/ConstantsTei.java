...
/**
 * Implementation of <code>TagExtraInfo</code> for the <b>constants</b> tag, identifying the
    ...
    try {
      ...
      if (data.getAttributeString("var") == null) {
        ...
        for (final Field field : fields) {
          ...
          vars.add(new VariableInfo(field.getName(), ((field.getType().isArray()) ? type.substring(
            2, type.length() - 1) + "[]" : type), true, VariableInfo.AT_END));
        }
      }
      else {
