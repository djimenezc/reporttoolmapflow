package com.mapflow.geo.common.dao.spring;
...
/**
 * <p>
 * Adds Hibernate persistent class definitions to an existing Spring Session Factory bean, possibly
 * defined within a seperate Spring configuration file in a seperate jar file. By using this
 * extension factory developers can add persistent classes to an AppFuse application without
 * modifying any of the existing AppFuse Spring configuration or jar distribution files.
 * <p>
 ...
 * </pre>
 * <p>
 * The snippet will add two persistent classes to an existing Session Factory bean called
 * &quot;sessionFactory&quot;. Note that the extension can handle both annotated classes and the
 * more traditional .hbm.xml files. Assuming that these persistent classes are packaged in a jar
 * called extension.jar which contains the Spring configuration file applicationContext-dao.xml at
 * the root level, then the standard AppFuse configuration will automatically pick up the new Spring
 * configuration file and the new persistent classes will be added to the list already defined for
 * the session factory bean configured within the standard appfuse-hibernate.jar file. And all this
 * without needing to touch the original AppFuse configuration files!
 * 
...
public class HibernateExtensionPostProcessor implements BeanFactoryPostProcessor {
  ...
  public void postProcessBeanFactory(
    ...
    if (configurableListableBeanFactory.containsBean(sessionFactoryBeanName)) {
      ...
      if (hibernateProperties != null) {
        ...
        final Properties existingHibernateProperties = (Properties) propertyValue.getValue();
  ...
  /**
   * The list of configuration locations (i.e. classpath:hibernate.cfg.xml) to add to the session
   * factory
   * 
